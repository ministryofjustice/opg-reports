...
const ENDPOINT_BASE string = `/v1/accounts/`
const ENDPOINT_TEAM string = `/v1/accounts/team/{team}/`
...
func Register(ctx context.Context, mux *http.ServeMux, config *apimodels.Args) {
	...
	for _, ep := range endpoints {
		log.Info(fmt.Sprintf("[%s] registering endpoint [%s] to handler", "accountapiall", ep))
		ep = fmt.Sprintf("%s{$}", ep)
