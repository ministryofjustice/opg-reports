...
// changed the accounts table to be agnostic about aws and add
// vendor information. Also redo indexes
const accountsAwsAgnostic string = `
...
// original cost table and indexes
const costs string = `
...
CREATE INDEX IF NOT EXISTS infracosts_date_idx ON infracosts(date);
CREATE INDEX IF NOT EXISTS infracosts_date_account_idx ON infracosts(date, account_id);
CREATE INDEX IF NOT EXISTS infracosts_unique_idx ON infracosts(account_id,date,region,service);

`

// var MIGRATION_STATEMENTS = []string{
// 	// teams
// 	teams,
// 	// accounts
// 	accounts,
