...
func Handler(ctx context.Context, args *frontmodels.RegisterArgs, writer http.ResponseWriter, request *http.Request) {
	var (
		...
		templateName string         = "home-codeownership"
		log          *slog.Logger   = cntxt.GetLogger(ctx).With("package", "homecodeownership", "func", "Handler", "url", request.URL.String())
