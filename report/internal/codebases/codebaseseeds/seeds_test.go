...
import (
	...
	"fmt"
	"log/slog"
	"opg-reports/report/internal/codebases/codebasemigrations"
...
func TestRedoCodebasesSeedWorking(t *testing.T) {
	var (
		err        error
		db         *sqlx.DB
		...
		connStr    string          = fmt.Sprintf("%s/%s", dir, "seed-codebases-working.db")
	...
	if statements[0].Returned <= 0 {
		t.Errorf("expected positive id row insert")
	}
}
