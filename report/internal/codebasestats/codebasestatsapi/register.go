...
const ENDPOINT_BASE string = `/v1/codebase-stats/`
const ENDPOINT_TEAM string = `/v1/codebase-stats/team/{team}/`
...
func Register(ctx context.Context, mux *http.ServeMux, config *apimodels.Args) {
	...
	for _, ep := range endpoints {
		log.Info(fmt.Sprintf("[%s] registering endpoint [%s] to handler", "codebasestatsapi", ep))
		ep = fmt.Sprintf("%s{$}", ep)
