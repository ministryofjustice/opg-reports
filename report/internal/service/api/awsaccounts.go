...
type AwsAccountsGetter[T Model] interface {
	Closer
	GetAllAwsAccounts(store sqlr.RepositoryReader) (accounts []T, err error)
}
...
func (self *Service[T]) GetAllAwsAccounts(store sqlr.RepositoryReader) (accounts []T, err error) {
