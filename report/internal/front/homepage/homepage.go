...
func handler(ctx context.Context, log *slog.Logger, conf *Conf, writer http.ResponseWriter, request *http.Request) {
	...
	blockFs = []pageBlockF{
		...
		func(i ...any) {
			...
			var uptime, headings, err = blocks.UptimeData(ctx, log, conf.ApiHost, request, &api.Param{
				Type:   api.QUERY,
				Key:    "team",
				Value:  "true",
				Locked: true,
		...
		func(i ...any) {
			// lock in team filter = true
			var costs, headings, err = blocks.InfracostData(ctx, log, conf.ApiHost, request, &api.Param{
				Type:   api.QUERY,
				Key:    "team",
				Value:  "true",
				Locked: true,
		...
		// get codeowners
		func(i ...any) {
			var code, err = blocks.CodeownerData(ctx, log, conf.ApiHost, request, &api.Param{
				Type:   api.QUERY,
				Key:    "codeowner",
				Value:  "none",
				Locked: true,
