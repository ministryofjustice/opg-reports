...
const ENDPOINT_BASE string = `/home/costs/accounts/`
const ENDPOINT_TEAM string = `/team/{team}/costs/accounts/`
...
func Register(ctx context.Context, mux *http.ServeMux, args *frontmodels.RegisterArgs) {
	...
	for _, ep := range endpoints {
		log.Info(fmt.Sprintf("[%s] registering endpoint [%s] to handler", "costsbyaccounts", ep))
		ep = fmt.Sprintf("%s{$}", ep)
