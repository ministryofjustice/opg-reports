...
func Handler(ctx context.Context, args *Args, writer http.ResponseWriter, request *http.Request) {
	var (
		...
		log          *slog.Logger   = cntxt.GetLogger(ctx).With("package", "home", "func", "costsdiff", "url", request.URL.String())
	...
	wg.Wait()
	page.CostData.ExcludeFooter = true
	page.CostData.Months = times.AsYMStrings(times.Months(startDate, endDate))
	for i := 100; i <= 3000; i += 100 {
		page.CostData.Changes = append(page.CostData.Changes, fmt.Sprintf("%d", i))
	}
	// respond
...
func dataCallers(ctx context.Context, args *Args, request *http.Request) []dataCallerF {
	return []dataCallerF{
		...
		// get homepage stats
		func(wg *sync.WaitGroup, page *PageContent) {
			if costs, err := costapidiff.Get(ctx, args.ApiHost, request); err == nil {
				page.CostData = costs
			}
			wg.Done()
		},
