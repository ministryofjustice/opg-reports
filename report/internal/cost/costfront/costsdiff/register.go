...
const ENDPOINT_BASE string = `/home/costs/differences/`
const ENDPOINT_TEAM string = `/team/{team}/costs/differences/`
...
func Register(ctx context.Context, mux *http.ServeMux, args *frontmodels.RegisterArgs) {
	...
	for _, ep := range endpoints {
		log.Info(fmt.Sprintf("[%s] registering endpoint [%s] to handler", "costsdiff", ep))
		ep = fmt.Sprintf("%s{$}", ep)
