...
func TestDomainUptimeCLIMocked(t *testing.T) {
	var (
		err    error
		db     *sqlx.DB
	...
	yesterday := times.Yesterday()
	err = getAndImport(ctx, log, client, db, &options{
		AccountID: awsid.AccountID(ctx, log, region),
		DateEnd:   yesterday,
		DateStart: times.Ago(times.ResetMonth(yesterday), 1, times.MONTH),
	})
	if err != nil {
...
func TestDomainUptimeCLIWithoutMock(t *testing.T) {
	...
	var (
		...
		region string          = "us-east-1"
		ctx    context.Context = t.Context()
		log    *slog.Logger    = logger.New("info")
		dir    string          = t.TempDir()
