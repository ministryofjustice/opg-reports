...
var querySegments = map[string][]*qb.Segment{
	"_default": {
		{Type: qb.SELECT, Stmt: `base.id`},
		{Type: qb.SELECT, Stmt: `base.name`},
		{Type: qb.SELECT, Stmt: `base.environment`},
		{Type: qb.SELECT, Stmt: `base.label`},
		{Type: qb.SELECT, Stmt: `base.team_name`},
		{Type: qb.ORDERBY, Stmt: `base.team_name ASC`},
		{Type: qb.ORDERBY, Stmt: `base.name ASC`},
		{Type: qb.ORDERBY, Stmt: `base.environment ASC`},
	},
	"team": {
		{Type: qb.WHERE, Stmt: `base.team_name = :team`},
	},
	"environment": {
		{Type: qb.WHERE, Stmt: `base.environment = :environment`},
	},
}

// the query builder
var builder = &qb.Builder{
	From:     "accounts as base",
