...
const (
	...
	short string = `accounts fetches and imports account data from opg-metadata releases [needs GITHUB_TOKEN].`
)
...
func getAndImport(ctx context.Context, log *slog.Logger, client accountgetter.GitHubClient, db *sqlx.DB, params *options) (err error) {
	var (
		dir    string
		result []*dbstmts.Insert[*accountmodels.Account, string]
		data   []*accountmodels.Account = []*accountmodels.Account{}
		lg     *slog.Logger             = log.With("func", "accountcli.getAndImport", "tag", params.Tag)
	)

	lg.With("params", params).Info("starting account import ...")
	dir, _ = os.MkdirTemp("", "__import-accounts-*")
	// run the data getter command
	data, err = accountgetter.GetAwsAccountData(ctx, log, client, &accountgetter.Options{
		Tag:           params.Tag,
		DataDirectory: dir,
	})
	if err != nil {
