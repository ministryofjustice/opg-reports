...
func TestDomainAccountsCLIMocked(t *testing.T) {
	var (
		err    error
		db     *sqlx.DB
	...
	err = getAndImport(ctx, log, client, db, &options{Tag: "v0.1.26"})
	if err != nil {
...
func TestDomainAccountsCLIWithoutMock(t *testing.T) {
	...
	err = getAndImport(ctx, log, client.Repositories, db, &options{Tag: "v0.1.26"})
	if err != nil {
		t.Errorf("unexpected import error: [%s]", err.Error())
		t.FailNow()
	}
