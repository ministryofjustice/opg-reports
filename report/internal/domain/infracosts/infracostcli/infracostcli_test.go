...
func TestDomainInfracostCLIMocked(t *testing.T) {
	var (
		err    error
		db     *sqlx.DB
		...
		dbPath string               = filepath.Join(dir, "test-cli-mock-infracosts.db")
	)
	// set the database
	...
	err = getAndImport(ctx, log, client, db, &options{
		AccountID: awsid.AccountID(ctx, log, region),
		DateStart: times.MustFromString("2025-11-01"),
		DateEnd:   times.MustFromString("2026-01-12"),
	})
...
func TestDomainInfracostCLIWithoutMock(t *testing.T) {
	...
	var (
		...
		log    *slog.Logger    = logger.New("error")
		dir    string          = t.TempDir()
		dbPath string          = filepath.Join(dir, "test-cli-infracosts.db")
	)
	// set the database
	db, err = dbconnection.Connection(ctx, log, "sqlite3", dbPath)
	if err != nil {
