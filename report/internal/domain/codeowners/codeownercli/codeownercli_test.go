...
func TestDomainCodeownersCLIWithoutMock(t *testing.T) {
	...
	err = getAndImport(ctx, log, client.Repositories, db, &options{
		Codebases: code,
		Parent:    "opg",
...
func TestDomainCodeownersCLIMocked(t *testing.T) {
	var (
		err    error
		db     *sqlx.DB
